...
- skip 결정 규칙 (순서대로 확인):
  ★★ [최우선 skip=true 규칙] 아래 패턴에 해당하면 다른 규칙보다 먼저 skip=true로 확정 ★★
  0. 미디어/콘텐츠 앱 알림 → skip=true (아래 1~5번 규칙보다 우선 적용)
     - YouTube, 유튜브 영상/쇼츠 알림: "채널명: 영상제목" 또는 "님이 새 동영상을 게시했습니다" 형태
     - 유튜브 채널명은 인물 이름이 아님. "조코딩", "웅플", "빠니보틀" 등은 사람 이름이 아니라 콘텐츠 채널명
     - 앱 업데이트/설치 알림: "업데이트 가능", "최신 버전", "새 버전 출시"
     - 광고/마케팅 알림: "할인", "세일", "이벤트", "특가", "쿠폰" 등 쇼핑 판촉 내용
     - 시스템/운영 알림: AlertNotificationService, 앱 알림 서비스, 플랫폼 자동 발송 메시지
     - SNS 좋아요/팔로우/조회 알림: "회원님의 게시물", "님이 좋아요를 눌렀습니다", "새 팔로워"
     - 뉴스레터/정기 구독 메일: "구독", "뉴스레터", "주간 소식"
  ★★ [강제 skip=false 규칙] 아래 5가지 중 하나라도 해당하면 무조건 즉시 skip=false. 예외 없음. ★★
  (단, 위 0번 최우선 규칙이 먼저 적용되었다면 아래 규칙은 무시)
  ...
  5. 인물 이름(성명) 언급 → false
     ...
     - ★예외: YouTube/미디어 채널명(예: "조코딩", "웅플", "슈카월드")은 인물 이름이 아님 → 이 규칙 적용 안 함
